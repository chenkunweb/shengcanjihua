...
<script>
  ...
  export default {
    ...
    data: function() {
      return {
        ...
        hotSettings: {
            ...
            colHeaders:['生产日期', '工单号', '产品型号','镀种', '数量PCS','数量KG','完成数量','取消数量','交期', '备注'],//自定义列表头or 布尔值
         ...
         //加工类型
          this.$api.get('/api/JGTYPE','',res=>{
              JSON.parse(res.data).Table.forEach((item)=>{
                  this.hotSettings.columns[3].source.push(item.加工类型)
              })
          })
        //   最大编号获取
        this.$api.get('/api/Planmaxbh',{bm:'DY'},res=>{
            this.maxyd = JSON.parse(res.data).Table[0].最大工单号==null?0:JSON.parse(res.data).Table[0].最大工单号
        })
    ...
    methods:{
        ...
        //背景颜色选择器
        headleChangeBGColor(e){let a = 'BG';let b = 'BG,';this.util.style(a,b,e,_this);this.util.headleChangeBGColor(_this,e,'dd',1); _this.a.render()},
        //字体颜色选择器
        headleChangeColor(e){let a = 'color';let b = 'color,';this.util.style(a,b,e,_this);this.util.headleChangeColor(_this,e,'dd',1); _this.a.render()},
        //加粗
        blod(e){let a = 'blod';let b = 'blod,';this.util.style(a,b,e,_this);this.util.blod(_this,e,'dd',1); _this.a.render()},
        // 字体大小
        size(e){let a = 'size';let b = 'size,';this.util.style(a,b,e,_this);this.util.size(_this,e,'dd',1); _this.a.render()},
        
        // 保存文件
        down(){_this.util.down(this,'http://www.zanty.net:8012?date=')},
