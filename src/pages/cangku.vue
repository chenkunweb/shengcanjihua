  ...
  export default {
    ...
    data: function() {
      return {
        ...
        hotSettings: {
            ...
            colHeaders:['发货日期', '发货号', '产品型号', '计划数量','计划重量','镀种','实际发货','取消数量', '交期','是否电镀'],//自定义列表头or 布尔值
            enterMoves: {row: 0, col: 1},// 标题筛选
            ...
            columns: [//添加每一列的数据类型和一些配置
                ...
                {data: 'shuliang'}, //数量
                {data: 'danwei'}, //单位
                {data:'type',type: "autocomplete",filter: true,source: []},//镀种
                {data: 'wancheng'}, //完成数量
                {data: 'quxiao'}, //取消数量
            ...
            cells: function(row, col, prop) {
                ...
                _this.$store.state.label.forEach((item)=>{//权限验证
                    if(item.name =='仓库'){
                ...
                if(changes){
                    ...
                    changes.forEach(([row,prop,oldValue,newValue]) => {
                        ...
                        if(prop == 'gongdan'&newValue!=''){//工单验证
                            ...
                            this.getSourceData()[row].caozuoshijian = time
                            this.getSourceData()[row].diandu = '电镀'
    ...
    created(){
        this.$store.state.path = window.location.hash.split('/')[1]//记忆路由
         ...
         //加工类型
          this.$api.get('/api/JGTYPE','',res=>{
              JSON.parse(res.data).Table.forEach((item)=>{
                  this.hotSettings.columns[5].source.push(item.加工类型)
              })
          })
        //样品最大编号
    ...
    methods:{
        ...
        //背景颜色选择器
        headleChangeBGColor(e){let a = 'BG';let b = 'BG,';this.util.style(a,b,e,_this);this.util.headleChangeBGColor(_this,e,'cangku',1);_this.a.render()},
        //字体颜色选择器
        headleChangeColor(e){let a = 'color';let b = 'color,';this.util.style(a,b,e,_this);this.util.headleChangeColor(_this,e,'cangku',1);_this.a.render()},
        //加粗
        blod(e){let a = 'blod';let b = 'blod,';this.util.style(a,b,e,_this);this.util.blod(_this,e,'cangku',1);_this.a.render()},
        // 字体大小
        size(e){let a = 'size';let b = 'size,';this.util.style(a,b,e,_this);this.util.size(_this,e,'cangku',1);_this.a.render()},
        // 保存文件
        //保存
       ...
       click(){this.util.click(_this,this.title) },
        ...
        // 日期筛选
        change(e){
            _this.util.change(e,this,'cangku')
            this.maxyps=0
            //样品最大编号
        this.$api.get('/api/Planmaxbh',{bm:'C'},res=>{
            this.maxyp = JSON.parse(res.data).Table[0].最大工单号==null?0:JSON.parse(res.data).Table[0].最大工单号
