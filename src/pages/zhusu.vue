...
export default {
    ...
    data(){
        return{
            ...
            hotSettings: {
                ...
                colHeaders:['生产日期', '工单号', '批号', '产品型号', '数量','完成数量','取消数量', '交期','零件状态',  '备注'],//自定义列表头or 布尔值
                ...
                beforeCopy:function(data,coords){
                    ...
                    let a =  data.map((item)=>{
                        let arr = item.map((item,index)=>{
                            ...
                            if(cols==0){type='riqi'}
                            if(cols==1){type='gongdan'}
                            if(cols==2){type='pihao'}
                            if(cols==3){type='xinghao'}
                            if(cols==4){type='shuliang'}
                            if(cols==5){type='wancheng'}
                            if(cols==6){type='quxiao'}
                            if(cols==7){type='jiaoqi'}
                            if(cols==8){type='zhuangtai'}
                            if(cols==9){type='beizhu'}
                            return {
                                    name:item,
                                    type:type
                                }
                ...
                afterCreateRow:function(index,amount,source){
                    if(this.copyData!=undefined){
                        _this.util.afterCreateRow(this,index)
                    }
                },
                // 初始化判断权限
    ...
    methods:{
        ...
        //背景颜色选择器
        headleChangeBGColor(e){let a = 'BG';let b = 'BG,';this.util.style(a,b,e,_this);this.util.headleChangeBGColor(_this,e,'zs',1);_this.a.render()},
        //字体颜色选择器
        headleChangeColor(e){let a = 'color';let b = 'color,';this.util.style(a,b,e,_this);this.util.headleChangeColor(_this,e,'zs',1);_this.a.render()},
        //加粗
        blod(e){let a = 'blod';let b = 'blod,';this.util.style(a,b,e,_this);this.util.blod(_this,e,'zs',1);_this.a.render()},
        // 字体大小
        size(e){let a = 'size';let b = 'size,';this.util.style(a,b,e,_this);this.util.size(_this,e,'zs',1);_this.a.render()},
        // 下载文件
        ...
        //保存
        down(){_this.util.down(this,'http://www.zanty.net:8018?date=')},
      
     
        //日期查询
        change(e){
            _this.util.change(e,this,'zs')
            this.maxyps = 0
           this.maxyds = 0
            // 样品最大编号
            this.$api.get('/api/Plangetp','',res=>{
