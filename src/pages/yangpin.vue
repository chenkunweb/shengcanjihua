...
<script>
  ...
  export default {
    ...
    data: function() {
      return {
        ...
        hotSettings: {
            ...
            colHeaders:['生产日期', '订单号', '产品型号', '数量','完成数量','取消数量', '交期','申请人','工程师', '散件欠料情况'],//自定义列表头or 布尔值
    ...
    created(){
        ...
        //样品最大编号
        this.$api.get('/api/Planmaxbh',{bm:'Y'},res=>{
    ...
    methods:{
        ...
        //背景颜色选择器
        headleChangeBGColor(e){let a = 'BG';let b = 'BG,';this.util.style(a,b,e,_this);this.util.headleChangeBGColor(_this,e,'yangpin',1);_this.a.render()},
        //字体颜色选择器
        headleChangeColor(e){let a = 'color';let b = 'color,';this.util.style(a,b,e,_this);this.util.headleChangeColor(_this,e,'yangpin',1);_this.a.render()},
        //加粗
        blod(e){let a = 'blod';let b = 'blod,';this.util.style(a,b,e,_this);this.util.blod(_this,e,'yangpin',1);_this.a.render()},
        // 字体大小
        size(e){let a = 'size';let b = 'size,';this.util.style(a,b,e,_this);this.util.size(_this,e,'yangpin',1);_this.a.render()},
        // 保存文件
        //保存
       ...
       click(){this.util.click(_this,this.title) },
        ...
        // 日期筛选
        change(e){
            _this.util.change(e,this,'yangpin')
            this.maxyps=0
            //样品最大编号
        this.$api.get('/api/Planmaxbh',{bm:'Y'},res=>{
            this.maxyp = JSON.parse(res.data).Table[0].最大工单号==null?0:JSON.parse(res.data).Table[0].最大工单号
