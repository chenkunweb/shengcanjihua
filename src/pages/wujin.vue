...
export default {
    ...
    data(){
        ...
        hotSettings: {
            ...
            colHeaders:['生产日期', '工单号', '批号', '产品型号', '数量','完成数量','取消数量', '交期','是否电镀', '零件状态', '备注'],//自定义列表头or 布尔值
            // hiddenColumns: {columns: [9,10,11,12,13,14,15],indicators: true},//隐藏单元列
            ...
            cells: function(row, col, prop) {
                ...
                if(this.instance.getSourceData()[row].times<time&prop!='wancheng'&prop!='quxiao'&prop!='diandu'){
                        cellProperties.readOnly= true
                    }
                if(prop=='wancheng'){
                    cellProperties.readOnly= true
                ...
                // console.log(this.instance.getSourceData()[row])
                return cellProperties;
            },
            // 样式
            afterSelectionEnd: function(row, column,row2,column2){
                 _this.util.afterSelectionEnd(this,_this,row, column,row2,column2)
    ...
    methods:{
        ...
        //限制数量开关
        bush(e){
            if(e==false){_this.headleChangeBGColor('#ffffff')}
             this.a.render()
        },
        // 表格查找本页数据函数
        contentAll(){
        ...
        headleChangeBGColor(e){
            console.log(e)
            let a = 'BG';let b = 'BG,';this.util.style(a,b,e,_this);this.util.headleChangeBGColor(_this,e,'wj',1);_this.a.render()},
        //字体颜色选择器
        headleChangeColor(e){let a = 'color';let b = 'color,';this.util.style(a,b,e,_this);this.util.headleChangeColor(_this,e,'wj',1);_this.a.render()},
        //加粗
        blod(e){let a = 'blod';let b = 'blod,';this.util.style(a,b,e,_this);this.util.blod(_this,e,'wj',1);_this.a.render()},
        // 字体大小
        size(e){let a = 'size';let b = 'size,';this.util.style(a,b,e,_this);this.util.size(_this,e,'wj',1);_this.a.render()},
        // 下载文件
        click(){this.util.click(_this,this.title)},
        // 清空查询数据
        clerks(){location.reload();},
        // 清空内容
        clerk(){this.value1 = '';_this.a.loadData(null)},
        //保存
        down(){_this.util.down(this,'http://www.zanty.net:8016?date=')},
        //日期查询
        change(e){
            _this.util.change(e,this,'wj')
            this.maxyps = 0
            this.maxyds = 0
            // 样品最大编号
            this.$api.get('/api/Plangetp','',res=>{
