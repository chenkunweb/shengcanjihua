...
<script>
  ...
  export default {
                    ...
                    changes.forEach(([row,prop,oldValue,newValue]) => {
                        ...
                        if(prop == 'gongdan'&newValue!=''){
                            ...
                            _this.Tablelist.forEach((item)=>{
                                if(newValue.split('%')[0].replace(/\s+/g, "") == item.订单号.replace(/\s+/g, "")){
                                    list = item
                                }
                            })
                            console.log(list)
                           if(list){//初始化数据赋值
                                this.getSourceData()[row].gongdan = list.订单号
                                this.getSourceData()[row].pihao = list.生产批号
                                this.getSourceData()[row].xinghao = list.物料说明
                                this.getSourceData()[row].daima = list.物料
                                this.getSourceData()[row].maxnums = list.数量*1-list.已排产*1+list.已取消*1
                                this.getSourceData()[row].shuliang = list.数量*1-list.已排产*1+list.已取消*1
                                this.getSourceData()[row].id = list.记录号
                           }
                           
                            
                        }
                        if(prop == 'shuliang'){//数量输入验证
                            if(reg.test(newValue)== true){
                                _this.errRow = row
                                let maxnums = this.getSourceData()[row].maxnums
                                console.log(maxnums)
    ...
    // 实例创建前
    beforeCreate(){ _this = this; },//this赋值
    // 实例创建完
    created(){
        this.$store.state.path = window.location.hash.split('/')[1]//记忆路由
        this.util.GetData(this)//初始化数据抓取
    },
    // 方法
    methods:{
        //取消数量计算
        ...
        //背景颜色选择器
        headleChangeBGColor(e){let a = 'BG';let b = 'BG,';this.util.style(a,b,e,_this);this.util.headleChangeBGColor(_this,e,'caigou',1);_this.a.render()},
        //字体颜色选择器
        headleChangeColor(e){let a = 'color';let b = 'color,';this.util.style(a,b,e,_this);this.util.headleChangeColor(_this,e,'caigou',1);_this.a.render()},
        //加粗
        blod(e){let a = 'blod';let b = 'blod,';this.util.style(a,b,e,_this);this.util.blod(_this,e,'caigou',1);_this.a.render()},
        // 字体大小
        size(e){let a = 'size';let b = 'size,';this.util.style(a,b,e,_this);this.util.size(_this,e,'caigou',1);_this.a.render()},
        // 保存文件
        //保存
        down(){_this.util.down(this,'http://www.zanty.net:8123?date=')},
       ...
       click(){this.util.click(_this,this.title) },
        ...
        // 日期筛选
        change(e){
            _this.util.change(e,this,'caigou')
            this.maxyps=0
            //样品最大编号
            this.$api.get('/api/Planmaxbh',{bm:'C'},res=>{
